...
class AccountConfig:
	...
	def from_dict(cls, data: dict, index: int) -> 'AccountConfig':
		"""从字典创建 AccountConfig"""
		provider = data.get('provider', 'agentrouter')
	...
	def has_github_credentials(self) -> bool:
		"""是否配置了 GitHub OAuth 登录凭据"""
		return bool(self.github_username and self.github_password)
	...
	try:
		...
		for i, account_dict in enumerate(accounts_data):
			...
			if 'api_user' not in account_dict:
				...
				has_github = account_dict.get('github_username') and account_dict.get('github_password')
				if not has_login and not has_github:
					print(f'ERROR: Account {i + 1} missing required field (api_user) - only browser login can omit it')
					return None
			...
			if not has_cookies and not has_login and not has_github:
				print(f'ERROR: Account {i + 1} must have cookies, email+password, or github_username+github_password')
				return None
